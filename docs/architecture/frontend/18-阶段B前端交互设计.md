...
interface ClarificationOptionsProps {
  ...
  customInput: string;  // [方案C] 自定义输入内容
  onSelect: (optionId: number | null, refinedQuery: string | null) => void;
  onCustomInput: (input: string) => void;  // [方案C] 自定义输入回调
}
...
**[方案C] 自定义输入区域**：
- 默认折叠，点击"展开自定义补充"按钮展开
- 文本框：最多200字符，placeholder："例如：220kV变电站室外隔离开关"
- 输入内容后自动选中"自定义补充"，取消其他选项
- 与系统选项互斥：选了自定义则其他选项取消，选了其他选项则清空自定义输入
...
interface QueryRequest {
  ...
  custom_refinement?: string | null; // [方案C] 用户自定义补充内容
...
**优先级规则**：
- `custom_refinement` 不为空时，优先使用自定义输入（覆盖 `refined_query`）
- `refined_query` 不为空时，使用系统生成的澄清选项
- 两者都为空时，使用原始 `query`
...
**选择澄清选项**：
- 单选，选中后其他选项变灰
- "跳过"选项：`selected_option_id=null`，`refined_query=null`，`custom_refinement=null`
- 系统选项：`selected_option_id=X`，`refined_query=选项的refined_query`，`custom_refinement=null`
- 自定义输入：`selected_option_id=null`，`refined_query=null`，`custom_refinement=用户输入内容`
...
**[方案C] 自定义输入行为**：
- 折叠状态默认，点击"展开自定义补充"展开
- 在自定义框输入内容时，自动取消所有系统选项的选中
- 选择任一系统选项时，自动清空自定义输入框
...
**2026-07-16 - [方案C] 自定义补充输入**：
- 澄清选项列表新增折叠式自定义输入区域
- QueryRequest 新增 `custom_refinement` 字段，优先级高于 `refined_query`
- 前端状态管理新增 `customRefinement` 和互斥逻辑
- 自定义输入与系统选项互斥：选一清空另一
