...
路由层：快车道 / 慢车道
  ↓
召回层：向量 + 关键词 + 结构化 → Top50
  ↓
重排层：粗排(Top50→Top20) → 精排(Top20→Top5)
  ↓
生成层：LLM生成 + 引用溯源
  ↓
返回答案
...
3. **查询改写**：对清晰查询生成2-3个同义专业问法
4. **元数据提取**：提取电压等级、标准号等过滤条件
