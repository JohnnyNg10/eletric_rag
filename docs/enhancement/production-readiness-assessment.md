...
### 1.1 生成层 (Generation Layer) - 🔴 严重问题

**文件**: `backend/app/core/generation/generator.py`
- **行 416, 426**: 异常处理使用空 `pass` 块，错误被静默吞没
- **行 259**: Token 估算使用简单近似算法，不适合生产环境
- **影响**: LLM 生成出错时无法感知或恢复，用户会收到不完整的响应

**文件**: `backend/app/core/generation/validator.py`
- **行 260, 268, 276**: JSON 提取失败时使用空 `pass` 块（共 3 处）
- **行 251-278**: JSON 解析仅依赖正则，缺乏鲁棒性
- **影响**: 事实验证可能静默失败，系统无法发现 LLM 幻觉
...
### 1.2 检索层 (Retrieval Layer) - 🟡 部分问题

**文件**: `backend/app/core/retrieval/fast_lane.py`
- **行 65-68**: 文档中标注 `TODO` 但功能实际已实现（误导性注释）：
  ```python
  # 三路并行召回（TODO）
  # 两阶段重排（TODO）
  # 充分性判断（TODO）
...
- **行 277, 286, 294**: JSON 提取失败时无错误恢复
- **行 289-296**: 正则提取 JSON 容易失败且静默
...
- **行 180, 188**: 空 `pass` 异常处理块（JSON 解码失败时）
- **行 91**: HyDE 功能标记 `TODO: 使用LLM生成假设的专业答案（200字）`
- **影响**: 查询改写错误无法记录或恢复
...
| 文件 | 行号 | 上下文 |
|------|------|--------|
| `sufficiency.py` | 276, 285, 294 | JSON 提取（实际行号，非文档声明的 277/286/294） |
| `query_rewriter.py` | 180, 188 | 查询改写 JSON 解码 |
| `generator.py` | 416, 426 | LLM 生成 JSON 解析 |
| `validator.py` | 260, 268, 276 | 事实验证 JSON 提取 |
| `ingestion_pipeline.py` | 258, 265 | 文档导入 |

**影响**: 所有这些位置的错误都被静默吞没，无日志、无恢复、无用户反馈。
...
2. **错误处理加固** (2 天)
   - [ ] 修复所有 10 处空 `pass` 块 → 添加日志 + 错误恢复
   - [ ] 为每个 `except` 块添加明确的日志记录
   - [ ] 在生成层添加完善的降级处理
