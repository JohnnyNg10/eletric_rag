...
**空 `pass` 块分布** (共 5 处，已修复 5 处):

| 文件 | 行号 | 上下文 | 状态 |
|------|------|--------|------|
| `sufficiency.py` | 276, 285, 294 | JSON 提取（原 3 处） | ✅ 已添加日志 |
| `query_rewriter.py` | 180, 188 | 查询改写 JSON 解码（原 2 处） | ✅ 已添加日志 |
| `generator.py` | 416, 426 | LLM 生成 JSON 解析 | ⚠️ 待修复 |
| `validator.py` | 260, 268, 276 | 事实验证 JSON 提取 | ⚠️ 待修复 |
| `ingestion_pipeline.py` | 258, 265 | 文档导入 | ⚠️ 待修复 |

**影响**: 已修复的 5 处现在会记录错误日志，剩余 7 处仍静默失败。
...
2. **错误处理加固** (1.5 天)
   - [x] 修复 5 处空 `pass` 块（sufficiency.py 3处 + query_rewriter.py 2处）→ 已添加日志
   - [x] 删除 query_rewriter.py 中冗余的 HyDE 死代码
   - [ ] 修复剩余 7 处空 `pass` 块（generator.py 2处 + validator.py 3处 + ingestion_pipeline.py 2处）
   - [ ] 在生成层添加完善的降级处理
