...
4. **图片讲解规则**（重要）：
   - 如果参考资料包含【图片描述】，这是对图示内容的文字描述，你需要：
     a) 在答案中说"如图所示"或"从图中可以看到"来引入图片
     b) 根据描述详细讲解图片中的关键元素、结构、流程
     c) 在讲解位置使用特殊标记 `[图片引用:N]` 表示图片插入位置（N为引用编号）
     d) 讲解完毕后标注引用来源[N]
   - 示例：
     * 不好：详见引用来源[2]中的配图
     * 好：充电设备接入方式如图所示[图片引用:2]，从图中可以看到，电源取自220V/380V低压配电箱，经过两级断路器保护后接入充电设备。上级断路器提供总开关和过载保护，下级断路器作为接入点连接充电桩。[2]
...
- 如参考资料确实不包含所问内容，明确说明
- 当参考资料包含图片时，主动讲解图片内容，不要只说"请查看配图"""
    ...
    def build_prompt(
        ...
        for i, chunk in enumerate(chunks, 1):
            ...
            # 图片描述 chunk：引导 LLM 详细讲解图片
            elif chunk.content_type == "image_description":
                # 标记这是图片描述，引导 LLM 使用 [图片引用:N] 标记
                content = f"【图片描述】\n{content}\n\n提示：这是一张图片的文字描述。在答案中引用此图片时，使用格式「如图所示[图片引用:{i}]」来插入图片，然后根据上述描述详细讲解图中的关键元素、结构或流程，最后标注引用来源[{i}]。"
            # 文本 chunk 引用了图片：提示 LLM 讲解相关图片
                ...
                if fig_numbers:
                    content += f"\n\n【此段落涉及{', '.join(fig_numbers)}】提示：在答案中讲解相关图片时，使用「如{fig_numbers[0]}所示[图片引用:{i}]」格式插入图片位置标记。"
...
- **重要**：如果参考资料包含【图片描述】，必须在答案中使用 `[图片引用:N]` 标记插入图片位置，并详细讲解图中内容，不要只说"请查看配图"
